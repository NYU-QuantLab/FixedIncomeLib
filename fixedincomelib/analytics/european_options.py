...
class EuropeanOptionAnalytics:
    ...
    def european_option_log_normal(
        ...
        calc_risk : Optional[bool]=False) -> Dict[SimpleMetrics, float]:
    ...
    def european_option_normal(
        ...
        # pricing
        if option_type == CallOrPut.CALL:
            res[SimpleMetrics.PV] = (forward - strike) * norm.cdf(d) + normal_sigma * sqrt_t * norm.pdf(d)
        elif option_type == CallOrPut.PUT:
            res[SimpleMetrics.PV] = (strike - forward) * norm.cdf(-d) + normal_sigma * sqrt_t * norm.pdf(d)
        else:
            raise ValueError("option_type must be 'call' or 'put'")
    ...
    def implied_lognormal_vol_sensitivities(
        ...
        tol: Optional[float]=1e-8) -> Dict[SimpleMetrics, float]:
        ...
        if calc_risk:
            res.update({
                SimpleMetrics.D_LN_VOL_D_FORWARD: - greeks[SimpleMetrics.DELTA] / greeks[SimpleMetrics.VEGA],
                SimpleMetrics.D_LN_VOL_D_TTE: - greeks[SimpleMetrics.TTE_RISK] / greeks[SimpleMetrics.VEGA],
                SimpleMetrics.D_LN_VOL_D_STRIKE: - greeks[SimpleMetrics.STRIKE_RISK] / greeks[SimpleMetrics.VEGA]
            })
    ...
    def implied_normal_vol_sensitivities(
        ...
        if calc_risk:
            res.update({
                SimpleMetrics.D_N_VOL_D_FORWARD: - greeks[SimpleMetrics.DELTA] / greeks[SimpleMetrics.VEGA],
                SimpleMetrics.D_N_VOL_D_TTE: - greeks[SimpleMetrics.TTE_RISK] / greeks[SimpleMetrics.VEGA],
                SimpleMetrics.D_N_VOL_D_STRIKE: - greeks[SimpleMetrics.STRIKE_RISK] / greeks[SimpleMetrics.VEGA]
            })
    ...
    def lognormal_vol_to_normal_vol(
        ...
        black_res = EuropeanOptionAnalytics.european_option_log_normal( \
            forward + shift, strike + shift, time_to_expiry, log_normal_sigma, option_type, calc_risk)
        ...
        if calc_risk:
            ...
            vega_res = EuropeanOptionAnalytics.european_option_normal( \
                forward + shift, strike + shift, time_to_expiry, bachelier_res[SimpleMetrics.IMPLIED_NORMAL_VOL], option_type, calc_risk)
            # vol risk
            res[SimpleMetrics.D_N_VOL_D_LN_VOL] = black_res[SimpleMetrics.VEGA] / vega_res[SimpleMetrics.VEGA]
            # forward risk
            res[SimpleMetrics.D_N_VOL_D_FORWARD] = bachelier_res[SimpleMetrics.D_N_VOL_D_FORWARD] + \
                1. / vega_res[SimpleMetrics.VEGA] * black_res[SimpleMetrics.DELTA]
            # strike risk
            res[SimpleMetrics.D_N_VOL_D_STRIKE] = bachelier_res[SimpleMetrics.D_N_VOL_D_STRIKE] + \
                1. / vega_res[SimpleMetrics.VEGA] * black_res[SimpleMetrics.STRIKE_RISK]
            # tte risk
            res[SimpleMetrics.D_N_VOL_D_TTE] = bachelier_res[SimpleMetrics.D_N_VOL_D_TTE] + \
                1. / vega_res[SimpleMetrics.VEGA] * black_res[SimpleMetrics.TTE_RISK]
    ...
    def normal_vol_to_lognormal_vol(
        ...
        if calc_risk:
            ...
            vega_res = EuropeanOptionAnalytics.european_option_log_normal( \
                forward + shift, strike + shift, time_to_expiry, black_res[SimpleMetrics.IMPLIED_LOG_NORMAL_VOL], option_type, calc_risk)
            # vol risk
            res[SimpleMetrics.D_LN_VOL_D_N_VOL] = bachelier_res[SimpleMetrics.VEGA] / vega_res[SimpleMetrics.VEGA]
            # forward risk
            res[SimpleMetrics.D_LN_VOL_D_FORWARD] = black_res[SimpleMetrics.D_LN_VOL_D_FORWARD] + \
                1. / vega_res[SimpleMetrics.VEGA] * bachelier_res[SimpleMetrics.DELTA]
            # strike risk
            res[SimpleMetrics.D_LN_VOL_D_STRIKE] = black_res[SimpleMetrics.D_LN_VOL_D_STRIKE] + \
                1. / vega_res[SimpleMetrics.VEGA] * bachelier_res[SimpleMetrics.STRIKE_RISK]
            # tte risk
            res[SimpleMetrics.D_LN_VOL_D_TTE] = black_res[SimpleMetrics.D_LN_VOL_D_TTE] + \
                1. / vega_res[SimpleMetrics.VEGA] * bachelier_res[SimpleMetrics.TTE_RISK]

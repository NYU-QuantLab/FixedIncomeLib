...
class ValutionParametersBuilderRegistry(Registry):
...
class ValuationParametersCollection:
    ...
    def deserialize(cls, input_dict : dict):
        ...
        for _, v in input_dict_.items():
            func = ValutionParametersBuilderRegistry().get(v['TYPE'])
...
class FundingIndexParameter(ValuationParameters):
    ...
    def __init__(self, content : Union[List, dict]):
        ...
        this_index = self.vp_dict_['FUNDING INDEX']
        self.funding_index_map_ = {}
        if this_index == '':
            assert self.vp_dict_['CURRENCIES'] != '' and self.vp_dict_['FUNDING INDICES'] != ''
            these_ccys = self.vp_dict_['CURRENCIES'].split(';')
            these_indices = self.vp_dict_['FUNDING INDICES'].split(';')
            assert len(these_ccys) == len(these_indices)
            for ccy, index in zip(these_ccys, these_indices):
                self.funding_index_map_[Currency(ccy)] = IndexRegistry().get(index)
        else:
            self.funding_index_ = IndexRegistry().get(this_index)

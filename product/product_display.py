...
from .product import Product, ProductVisitor
from .portfolio import ProductPortfolio
from .linear_products import (
...
from .non_linear_products import (
...
__all__ = ["ProductDisplayVisitor", "displayProduct"]
...
# ------------------------
# Functional API for users
# ------------------------
def displayProduct(product: Product, *, preview_children: int | None = None) -> pd.DataFrame:
    """
    One-liner function for users.
    Example:
        cf = ProductBulletCashflow("2025-12-31", "USD", 1e6, "LONG")
        df = displayProduct(cf)
    """
    v = ProductDisplayVisitor(preview_children=preview_children)
    product.accept(v)
    return v.display()
